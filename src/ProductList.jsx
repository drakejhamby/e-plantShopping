...
function ProductList({ onHomeClick }) {
    ...
    const handleAddToCart = (plant, categoryName) => {
        const plantWithId = {
            ...plant,
            id: `${categoryName}-${plant.name}`,
        };

        dispatch(addItem(plantWithId));
    };
    return (
        <div>
                    ...
                    <div>
                        <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
                            <h1 className='cart'>
                                ...
                                {cartTotalItems > 0 && (
                                    <span className="cart_quantity_count">{cartTotalItems}</span>
                                )}
                            </h1>
                        </a>
                    </div>
                    ...
                    {plantsArray.map((category, index) => (
                        ...
                        <div className="product-list">
                             {category.plants.map((plant, plantIndex) => (
                                (() => {
                                    const plantId = `${category.category}-${plant.name}`;
                                    const isAdded = cartItems.some((item) => item.id === plantId);
